...
class SolicitarAjusteDialog extends StatefulWidget {
  final DateTime? dataInicial;
  final List<RegistroPontoModel> todosRegistros;

  const SolicitarAjusteDialog({
    super.key,
    this.dataInicial,
  ...
  @override
  State<SolicitarAjusteDialog> createState() => _SolicitarAjusteDialogState();
}

class _SolicitarAjusteDialogState extends State<SolicitarAjusteDialog> {
  ...
  Future<void> _salvarSolicitacao() async {
    ...
    try {
      final sucesso = await pontoProvider.solicitarAjuste(
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              sucesso
                  ? 'Solicitação de ajuste enviada! Aguardando aprovação do RH.'
                  : 'Solicitação salva localmente. Será enviada quando houver conexão.',
            ),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao solicitar ajuste: $e'),
                ...
                children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Sua solicitação será enviada para aprovação do Gestor de RH',
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 12),
                      ElevatedButton.icon(
                        onPressed: _isEnviando ? null : _salvarSolicitacao,
