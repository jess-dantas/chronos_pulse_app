...
- **Termo de Ciência (LGPD) bloqueante:** `ConsentimentoGate` no `MainShell` consulta `GET /privacidade/consentimento/status` e, se o aceite da versão vigente estiver pendente, abre modal não-dispensável com "Ciente e de acordo (vX)" ou "Sair" (logout) — escopo apenas usuários do painel (Admin Plataforma fica de fora **e não possui a aba `/admin/privacidade`**).
